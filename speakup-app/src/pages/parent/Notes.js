                      ...
                      <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                        <Box flexGrow={1}>
                          <Box display="flex" alignItems="center" mb={1}>
                            <Typography variant="h6" sx={{ color: '#2C3E50', mr: 2 }}>
                              {note.title}
                            </Typography>
                            <Chip
                              label={noteTypes.find(t => t.value === note.type)?.label}
                              size="small"
                              sx={{ background: getTypeColor(note.type), color: 'white', fontWeight: 'bold', mr: 1 }}
                            />
                            <Chip
                              label={priorities.find(p => p.value === note.priority)?.label}
                              size="small"
                              sx={{ background: getPriorityColor(note.priority), color: 'white', fontWeight: 'bold' }}
                            />
                          </Box>
                        ...
                        <Box display="flex" gap={1}>
                          <IconButton size="small" onClick={() => handleEditNote(note)} sx={{ color: '#4ECDC4' }}>
                            <Edit size={16} />
                          </IconButton>
                          <IconButton size="small" onClick={() => handleDeleteNote(note.id)} sx={{ color: '#FF6B6B' }}>
                            <Delete size={16} />
                          </IconButton>
                        </Box>
                      </Box>
